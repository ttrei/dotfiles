...
}:
{
  ...
  programs.neovim = {
    enable = true;
    # Keep the pre-26.05 defaults explicit and silence Home Manager warnings.
    withRuby = true;
    withPython3 = true;
    # Home Manager creates ~/.config/nvim/init.lua; plugin-dependent config is loaded from after/plugin.
    initLua = ''
      require("reinis")
    '';
